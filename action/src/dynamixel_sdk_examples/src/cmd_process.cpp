...
auto message_unique = dynamixel_sdk_custom_interfaces::msg::SetPosition();
auto message_sync = dynamixel_sdk_custom_interfaces::msg::SetPositionSync();      

class Action_Editor_Pub : public rclcpp::Node
{
public:
    Action_Editor_Pub()
    : Node("cmd_node"), count_(0)
    {
    	publisher_ = this->create_publisher<dynamixel_sdk_custom_interfaces::msg::SetPosition>("set_unique_position", 10);    
		publisher_sync = this->create_publisher<dynamixel_sdk_custom_interfaces::msg::SetPositionSync>("set_position", 10);    
    }
    rclcpp::TimerBase::SharedPtr timer_;
    rclcpp::Publisher<dynamixel_sdk_custom_interfaces::msg::SetPosition>::SharedPtr publisher_;    
	rclcpp::Publisher<dynamixel_sdk_custom_interfaces::msg::SetPositionSync>::SharedPtr publisher_sync;      
    size_t count_;
	...
	void SetValue(int value)
	{
		...
		if( col == STP7_COL )
		{
			...
			{
				if(value  + MotionManager::GetInstance()->m_Offset[row + 1] >= 0 && value  + MotionManager::GetInstance()->m_Offset[row + 1] <= 4095)
				{
					int error;
					message_unique.id = row + 1;          
					message_unique.position = value  + MotionManager::GetInstance()->m_Offset[row + 1];
					publisher_->publish(message_unique);  
				}
				else
		...
		else if( col <= STP6_COL )
		{
			...
			{
				if(value  + MotionManager::GetInstance()->m_Offset[row + 1] >= 0 && value  + MotionManager::GetInstance()->m_Offset[row + 1] <= 4095) //Aumentando o valor da entrada
				{
	...
	void GoCmd(int index)
	{
		...
		int wGoalPosition, wStartPosition;
		uint8_t id_vector[];
		...
		for(id=JointData::ID_R_SHOULDER_PITCH; id<JointData::NUMBER_OF_JOINTS; id++)
		{
			...
			wGoalPosition[n] = Page.step[index].position[id] + MotionManager::GetInstance()->m_Offset[id];
			n++;
			//message.id[n++] = id;          
			//message.position[n++] = wGoalPosition; 
			for(id=JointData::ID_R_SHOULDER_PITCH; id<JointData::NUMBER_OF_JOINTS; id++){
				id_vector[id] = (u_int8_t) id;
			}

			for(id=JointData::ID_R_SHOULDER_PITCH; id<JointData::NUMBER_OF_JOINTS; id++){
				message_sync.id = id_vector;          
				message_sync.position = wGoalPosition; 
				publisher_sync->publish(message_sync);
			}
		}
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<Action_Editor_Pub>());
  rclcpp::shutdown();
  return 0;
}
