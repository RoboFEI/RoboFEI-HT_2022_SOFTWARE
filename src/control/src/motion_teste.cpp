...
public:
  ...
  : Node("gait_publisher")
  {
    RCLCPP_INFO(this->get_logger(), "Running motion managar");
    subscription_imu = this->create_subscription<sensor_msgs::msg::Imu>("imu/data", 10, std::bind(&topic_callback, this, _1));
	subscription_walk = this->create_subscription<custom_interfaces::msg::Walk>("walking", 10, std::bind(&topic_callback_walk, this, _1));
	subscription_neck = this->create_subscription<custom_interfaces::msg::NeckPosition>("/neck_position", 10, std::bind(&topic_callback_neck, this, _1));
    ...
    timer_ = this->create_wall_timer(4ms, std::bind(&Process, this));
	keep_walking = false;
	for(int i = 0; i < JointData::NUMBER_OF_JOINTS; i++)
        m_Offset[i] = 0;
	// update_thread_ = std::thread(std::bind(&update_loop, this));
	printf("CONSTRUTOR\n");
...
void topic_callback_walk(const std::shared_ptr<custom_interfaces::msg::Walk> walk_msg_) const
    {
		...
		if (GetInstance()->keep_walking==false && Walking::GetInstance()->m_Phase==0){
			...
			if(walk!=0){
				...
				GetInstance()->LoadINISettings(ini);
				Action::GetInstance()->Stop();
				//GetInstance()->AddModule((MotionModule*)Walking::GetInstance());
				...
				GetInstance()->keep_walking=true;
				//printf("KEEP WALKING DEPOIS %d\n", GetInstance()->keep_walking);
			}
			
			else{ // parar o walking
				printf("WALK PARADDO :( \n");
				if (GetInstance()->keep_walking==false && Walking::GetInstance()->GetCurrentPhase()==0){
					Action::GetInstance()->Stop();
					GetInstance()->AddModule((MotionModule*)Walking::GetInstance());
					GetInstance()->Initialize();
...
void LoadINISettings(minIni* ini, const std::string &section)
{
    ...
    for(int i = JointData::ID_MIN; i <= JointData::ID_MAX-2; i++)//tirando a cabeça
    {
        ...
        if((ivalue = ini->geti(section, key, INVALID_VALUE)) != INVALID_VALUE)  GetInstance()->m_Offset[i] = ivalue;
		printf("OFFSET %d\n", GetInstance()->m_Offset[i]);
    }
...
void Process()
{
	...
	if(walk!=0){
		...
		if(GetInstance()->m_ProcessEnable == false || GetInstance()->m_IsRunning == true)
			return;
			
		GetInstance()->m_IsRunning = true;



		GetInstance()->m_CalibrationStatus = 1;
		...
		if(GetInstance()->m_CalibrationStatus == 1 && GetInstance()->GetEnable() == true)
		{
			...
			for(int id=JointData::ID_MIN; id<=JointData::ID_MAX-2; id++) // loop que vai de 1 até 18
					{
					param[id] = id;
					pos[id] = MotionStatus::m_CurrentJoints.GetValue(id)+ GetInstance()->m_Offset[id];
